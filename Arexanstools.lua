...
loadstring(game:HttpGet("https://raw.githubusercontent.com/AREXANS/emoteff/refs/heads/main/emote.lua"))()
    ...
    local IsAntiLagEnabled = false -- Variabel baru untuk Anti-Lag
    local antiLagConnection = nil -- Koneksi baru untuk Anti-Lag
    ...
    -- [BARU] Frame Konten Tab Emote
    local EmoteTabContent = Instance.new("ScrollingFrame")
    EmoteTabContent.Name = "EmoteTab"
    EmoteTabContent.Size = UDim2.new(1, -10, 1, -10)
    EmoteTabContent.Position = UDim2.new(0, 5, 0, 5)
    EmoteTabContent.BackgroundTransparency = 1
    EmoteTabContent.Visible = false
    EmoteTabContent.CanvasSize = UDim2.new(0, 0, 0, 0)
    EmoteTabContent.ScrollBarThickness = 10
    EmoteTabContent.VerticalScrollBarInset = Enum.ScrollBarInset.Always
    EmoteTabContent.ScrollingDirection = Enum.ScrollingDirection.Y
    EmoteTabContent.Parent = ContentFrame
    ...
    setupCanvasSize(EmoteListLayout, EmoteTabContent) -- [BARU]
    ...
    local function switchTab(tabName)
        ...
        EmoteTabContent.Visible = (tabName == "Emote") -- [DIUBAH]
    ...
    local EmoteTabButton = createTabButton("Emote", TabsFrame) -- [BARU]
    ...
    local function ToggleAntiLag(enabled)
        ...
        if enabled then
            ...
            -- Nonaktifkan efek dan partikel yang ada
            ...
            -- Pantau dan nonaktifkan partikel/efek baru
            antiLagConnection = Workspace.DescendantAdded:Connect(function(descendant)
                if descendant:IsA("ParticleEmitter") or descendant:IsA("Explosion") or descendant:IsA("Fire") or descendant:IsA("Smoke") or descendant:IsA("Sparkles") then
                    task.wait() -- Tunggu sebentar agar properti bisa diakses
    ...
    createToggle(GeneralTabContent, "Anti-Lag", IsAntiLagEnabled, ToggleAntiLag) -- Tombol Anti-Lag baru
    ...
    -- [BARU] Tab Emote
    createToggle(EmoteTabContent, "Aktifkan Emote GUI", true, function(isEnabled)
        if getgenv().AFEM and getgenv().AFEM:IsA("ScreenGui") then
            getgenv().AFEM.Enabled = isEnabled
            showNotification("Emote GUI " .. (isEnabled and "diaktifkan" or "dinonaktifkan"), isEnabled and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 150, 50))
        else
            showNotification("GUI Emote (AFEM) tidak ditemukan. Pastikan skrip emote sudah dijalankan.", Color3.fromRGB(200, 50, 50))
        end
    end).LayoutOrder = 1
    
    local EmoteInfoLabel = Instance.new("TextLabel", EmoteTabContent)
    EmoteInfoLabel.Size = UDim2.new(1, 0, 0, 40)
    EmoteInfoLabel.BackgroundTransparency = 1
    EmoteInfoLabel.Text = "Fitur ini memerlukan skrip 'emote.lua' untuk dijalankan agar berfungsi."
    EmoteInfoLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
    EmoteInfoLabel.TextSize = 11
    EmoteInfoLabel.Font = Enum.Font.SourceSans
    EmoteInfoLabel.TextWrapped = true
    EmoteInfoLabel.LayoutOrder = 2
                     ...
                     if endedInput.UserInputType == input.UserInputType then
                        ...
                        if dragHandle == MiniToggleButton and not isDragging then
                            ...
                            if MainFrame.Visible then
                                if not (PlayerTabContent.Visible or GeneralTabContent.Visible or CombatTabContent.Visible or TeleportTabContent.Visible or SettingsTabContent.Visible or EmoteTabContent.Visible) then 
                                    switchTab("Player") 
                                else 
                                    updatePlayerList() 
                                end
